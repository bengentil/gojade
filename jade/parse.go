...
func (t *Tree) backupIndent(i int) {
	t.peekCount += i
}
...
func (t *Tree) parse(treeSet map[string]*Tree) (next Node) {
	...
	for t.peek().typ != itemEOF {
		switch t.peek().typ {
		case itemDoctype:
			t.Root.append(newDoctype(t.next().val))
		case itemTag:
			t.Root.append(t.parseTag(t.next().val, 0))
		case itemComment:
			t.next()
		case itemEndl:
			t.next()
...
func (t *Tree) parseTag(val string, level int) Node {
	tag := newTag(val)
Loop:
	for {

		if debug {
			fmt.Printf("%s:%s level=%d, t.indent=%d\n",
				t.peek().typ, t.peek().val, level, t.indent)
		}

		switch token := t.next(); token.typ {
		case itemId:
			tag.append(newId(token.val))
		case itemAttr:
			tag.append(newAttr(token.val))
		case itemClass:
			tag.append(newClass(token.val))
		case itemText:
			tag.append(newText(token.val))
		case itemIdentSpace:
			t.indent++
		case itemEndl:
			//tag.append(newText("\n"))
		...
		case itemTag:
			if t.indent > level {
				if token.val == "." || token.val == "#" {
					tag.append(t.parseTag("div", t.indent))
				} else {
					tag.append(t.parseTag(token.val, t.indent))
				}
			} else {
				t.backup()
				break Loop
